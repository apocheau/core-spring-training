...
public class AccountController {
	...
	public String addBeneficiary(@PathVariable("accountId") int accountId, @RequestParam String beneficiaryName) {
		accountManager.addBeneficiary(accountId, beneficiaryName);
		return "redirect:.";  // relative path prevents having to know servlet mapping
	}
	
	@RequestMapping(value="/accounts/{accountId}/beneficiaries/{beneficiaryName}", method=RequestMethod.DELETE)
	public String removeBeneficiary(@PathVariable("accountId") int accountId, @PathVariable("beneficiaryName") String beneficiaryName) {

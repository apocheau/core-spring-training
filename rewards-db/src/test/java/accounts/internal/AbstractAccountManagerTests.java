...
/**
 * Unit test for an account manager implementation.
 * <p>
...
public abstract class AbstractAccountManagerTests {
	...
	public abstract TransactionStatus getCurrentTransaction();
	
	@Test
	@Transactional
	public void testGetAllAccounts() {
		System.out.println("TRANSACTION IS : " + getCurrentTransaction());
	...
	public void addAccount() {
		// TODO: FIX ME - Manual configuration fails with JPA, but not raw
		//       Hibernate. Spring driven integration tests work fine.
		...
		Account newAccount = accountManager.getAccount(newAccountId);
		assertNotNull("Did not find new account", newAccount);
		assertEquals("Did not save account", "Test",
				newAccount.getName());
		assertEquals("Did not save beneficiary", 1,
				newAccount.getBeneficiaries().size());
	}
	...
	public void updateAccountBeneficiaries() {
		...
		accountManager.updateBeneficiaryAllocationPercentages(0L,
	...
	public void addBeneficiary() {
		...
		Account account = accountManager.getAccount(0L);
		assertEquals("Should only have three beneficiaries", 3, account
				.getBeneficiaries().size());
	}
	...
	public void removeBeneficiary() {
		...
		accountManager
				.removeBeneficiary(0L, "Annabelle", allocationPercentages);
		Account account = accountManager.getAccount(0L);
		assertEquals("Should only have one beneficiary", 1, account
				.getBeneficiaries().size());
		assertEquals("Corgan should now have 100% allocation",
				Percentage.oneHundred(), account.getBeneficiary("Corgan")
